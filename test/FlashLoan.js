...
describe("Flashloan", () => {
  let token, flashLoan, flashLoanReceiver, accounts;
  let deployer;
  ...
  beforeEach(async () => {
    ...
    // Deploy token
    token = await Token.deploy("Dapp University", "DAPP", 1000);
    ...
    let transaction = await token
      .connect(deployer)
      .approve(flashLoan.address, tokens(1000));
    await transaction.wait();

    // Deposit tokens into the pool
    transaction = await flashLoan.connect(deployer).depositTokens(tokens(1000));
    await transaction.wait();
  ...
  describe("Deployment", () => {
    it("sends tokens to the Crowdsale contract", async () => {
      expect(await token.balanceOf(flashLoan.address)).to.equal(tokens(1000));
    });
  });
